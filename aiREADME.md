...
- 內建測試列印頁 GET /test
- 
v20260606 完工：拿到完整 web + APK log 證據鏈，確認 NanoHTTPD parseBody 對 application/json 走 US-ASCII 導致中文 byte 進來就被吞成 ?。最終修法：PrintHttpServer.readBody 改為從 InputStream 直接讀 Content-Length byte，用 UTF-8 解碼，完全跳過 parseBody。
不做（已確認）：
- 項目 2、10、13：使用者明確排除
- 項目 12：自動更新檢查（維持手動更新）
