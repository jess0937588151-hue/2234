...
- [x] 新增 `js/modules/print-bridge.js`（三層橋接偵測：HTTP / WebView / window.print）
- [x] 改寫 `js/modules/print-service.js`（列印路由走 bridge，移除重複的 browserPrintHtml，printSessionReportViaBridge 改用 bridgeBrowserPrint）
- [x] 改寫 `js/pages/settings-page.js`（三區塊偵測改走 bridge，iPad/PC 顯示「系統列印模式」）
- [x] D9 線上訂單自動列印（接單時自動印廚房 + 顧客單，由 autoPrintKitchenOnConfirm / autoPrintReceiptOnConfirm 控制）
- [x] D10 預約 30 分鐘前自動提醒列印（`startReservationReminderLoop` 每 60 秒檢查，到時間彈 overlay 提示）

**第三階段：v20260602 列印優化**
- [x] `sunmiPrintReceiptByFont` 結尾留白縮為 1 行（原本 2 行）
- [x] `sunmiPrintReceiptByFont` 分隔線長度動態計算（依紙寬 58/70/80mm × 字體大小）
- [x] `buildPlainTextFromOrder` 修掉 `baseSize` ReferenceError、純依紙寬計算分隔線、結尾留白縮為 1 行
- [x] `pos-page.js` `finalizeOrder` 修掉多餘的 `}`，列印區塊正確包在函式內
- [x] `pos-page.js` 開錢箱改用 `openCashDrawer().catch(e=>...)` async 寫法
- [x] APK `SunmiPrinterManager.feedAndCut()` `lineWrap(4)` → `lineWrap(1)`
- [x] APK `SunmiPrinterManager.cutPaper()` 兩處 `lineWrap(3)` → `lineWrap(1)`
- [x] APK `SunmiPrinterManager.printPosReceipt()` 改成 `if (data.optBoolean("openDrawer", false)) openCashDrawer();` 條件開錢箱
- [x] `index.html` Sunmi、藍牙 modal 加測試列印按鈕
...
- 同時跑後台 NanoHTTPD on 127.0.0.1:8080，協定與 sunmi-pos-v2 完全相同
- 列印模組需含三類市售出單機：
...
app/src/main/java/com/pos/sunmiprinter/
  ...
  SettingsActivity.java              APK 設定頁（v20260603 後加 token / 列印狀態）
...
10. **APK CORS 不限制來源**（部署位置可能變動如私人 NAS），但**HTTP Server 必須綁 127.0.0.1** 防止 Wi-Fi 內任何裝置呼叫列印。
